        ...
        <div className="max-w-[25rem]">
          ...
          <ul className="max-w-[30rem]">
            {experienceContent.map((item) => (
              <li
                className="mb-5 p-0 md:p-3 lg:p-3 xl:p-3 rounded-md  lg:hover:bg-n-4/10 transition-all duration-200 lg:border-t border-transparent hover:border-n-4/30"
                key={item.id}
              >
                <div className="">
                  ...
                  <div className="flex flex-col">
                    ...
                    <small className="text-xs text-n-4 mt-2">{item.duration}</small>
                  </div>

                  <p className="mt-2 text-n-3 text-sm">{item.description}</p>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {item.tags.map((item) => (
                      <div className="text-xs rounded-full bg-indigo-500/20 px-2 py-1 text-indigo-500" key={item}>{item}</div>
                    ))}
                  </div>
                </div>
                {item.text && (
